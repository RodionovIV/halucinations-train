...
try:
    ...
    HAS_LGB = False
    print("[WARN] lightgbm not found – LightGBM model will be skipped.", file=sys.stderr)
...
def parse_args():
    ...
    p.add_argument("--output",      default="results_classical.csv",
                   help="Path for the results CSV")
    p.add_argument("--seed",        type=int, default=42)
    p.add_argument(
        "--feature_set", default=None,
        help=(
...
def main():
    ...
    if args.feature_set:
        if args.feature_set not in FEATURE_SETS:
            sys.exit(f"Unknown feature set '{args.feature_set}'. "
                     f"Choices: {list(FEATURE_SETS.keys())}")
